...
parser.add_argument('--input_file', default='./sample_test_baseline.txt',
                    help='input file')
parser.add_argument('--model_reap', default='./models/baseline.pt',
...
def main(args):
    ...
    for idx in range(0, len(sentences)):
        ...
        preds, _ = model_para(curr_inp.unsqueeze(0), curr_out.unsqueeze(0), None,
                              get_attention=False)
        preds = preds.view(-1, len(pp_vocab))
        preds = torch.nn.functional.log_softmax(preds, -1)
        nll = criterion(preds, curr_out_x.view(-1))

        gen = model_para.generate(curr_inp.unsqueeze(0), [list(bos)], None, beam_size=10,
