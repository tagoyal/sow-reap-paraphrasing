...
from sow.utils.tools import batch_sequences
from sow.utils.state import State
from sow.utils.config import PAD
from sow.utils.beam_search import SequenceGenerator
...
class Seq2Seq(nn.Module):
	...
	def encode(self, inputs, input_postags, input_pos_order = None, hidden=None, device_ids=None):
	...
	def forward(self, input_encoder, input_decoder, input_postags, input_pos_order, encoder_hidden=None, device_ids=None):
		if not isinstance(device_ids, dict):
			device_ids = {'encoder': device_ids, 'decoder': device_ids}
		context = self.encode(input_encoder, input_postags, input_pos_order, encoder_hidden,
	...
	def generate(self, input_encoder, input_postags, input_decoder, input_order, beam_size=None,
				 max_sequence_length=None, length_normalization_factor=0,
				 get_attention=False, device_ids=None):
		if not isinstance(device_ids, dict):
			device_ids = {'encoder': device_ids, 'decoder': device_ids}
		
		context = self.encode(input_encoder, input_postags, input_pos_order = input_order,
							  device_ids=device_ids.get('encoder', None))
		if hasattr(self, 'bridge'):
			state = self.bridge(context)
