...
from sow.models.seq2seq_base import Seq2Seq
from sow.utils.config import PAD
from sow.utils.state import State
...
class TransformerAttentionEncoder(nn.Module):
    ...
    def __init__(self, vocab_size, hidden_size=512, embedding_size=None,
                 num_layers=6, num_heads=8, inner_linear=256, inner_groups=1, prenormalized=False,
                 mask_symbol=PAD, batch_first=True, layer_norm=True, weight_norm=False, dropout=0, embedder=None, postag_size = 0):
        ...
        self.pos_embedder = nn.Embedding(postag_size, embedding_size, padding_idx=PAD)
        ...
        self.max_length = 100
        self.position_enc = nn.Embedding(self.max_length, inner_linear)
        self.position_enc.weight.data = self.position_encoding_init(self.max_length, inner_linear)
    ...
    def forward(self, inputs, input_postags, input_pos_order = None, hidden=None):
        ...
        if input_pos_order is not None:
            word_pos_encoded = self.position_enc(input_pos_order)
            x = x + word_pos_encoded
...
class TransformerAttentionDecoder(nn.Module):
    ...
    def __init__(self, vocab_size, hidden_size=512, embedding_size=None, num_layers=6, num_heads=8,
                 batch_first=True, dropout=0, inner_linear=256, inner_groups=1, prenormalized=False, stateful=None, state_dim=None,
    ...
    def forward(self, inputs, state, get_attention=True):
...
class Transformer(Seq2Seq):
    ...
    def __init__(self, vocab_size, hidden_size=512, embedding_size=None, num_layers=6, num_heads=8,
                 inner_linear=256, inner_groups=1, dropout=0.1, prenormalized=False, tie_embedding=True,
                 encoder=None, decoder=None, layer_norm=True, weight_norm=False, batch_first=True, stateful=None, postag_size = 0):
        ...
        encoder.setdefault('postag_size', postag_size)
